...
test::TestNormalMapping::TestNormalMapping()
	:m_Shader(nullptr), m_LightPos(0.5f, 1.0f, 0.3f)
{
	m_Camera = std::make_unique<Camera>(glm::vec3(0.0f, 0.0f, 3.0f));
	m_DiffuseMap = LoadImage("res/textures/brickwall.jpg");
	m_NormalMap = LoadImage("res/textures/brickwall_normal.jpg");

	// 着色器程序
	m_Shader = std::make_unique<Shader>("res/shaders/NormalMapping/Vertex.Vshader", "res/shaders/NormalMapping/Fragment.Fshader");
	m_Shader->Bind();
...
void test::TestNormalMapping::OnProcessMouseMovement(GLfloat xoffset, GLfloat yoffset)
{
	m_Camera->OnProcessMouseMovement(xoffset, yoffset);
}

void test::TestNormalMapping::OnProcessInput(GLFWwindow* window, GLfloat deltaTime)
{
	m_Camera->OnProcessInput(window, deltaTime);
}

int test::TestNormalMapping::LoadImage(char const* filename)
{
...
int test::TestNormalMapping::LoadImage(char const* filename, bool gammaCorrection)
{
	unsigned int textureID;
	GLCall(glGenTextures(1, &textureID));
...
void test::TestNormalMapping::RenderQuad()
{
	if (m_QuadVAO == 0)
	{
		// positions
...
void test::TestNormalMapping::OnUpdate(float deltaTime)
{
	
}

void test::TestNormalMapping::OnRender()
{
	...
	GLCall(glBindTexture(GL_TEXTURE_2D, m_NormalMap));
	RenderQuad();
...
void test::TestNormalMapping::OnImGuiRender()
{
